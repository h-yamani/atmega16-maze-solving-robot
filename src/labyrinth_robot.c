 ...
 *
 * Historical CodeVisionAVR firmware for a differential-drive robot equipped
 * with four obstacle sensors, two floor/path sensors, stepper motors, and a
 * 16-character LCD. The controller builds a small internal maze map, tracks
 * its heading, explores available cells, and stops when it reaches the
 * configured goal coordinates.
 *
 * Note: the control logic is intentionally preserved from the working
 * bachelor-project implementation. See docs/TECHNICAL_NOTES.md before making
 * behavioural changes or porting the program to another AVR toolchain.
 */
